...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(VVI_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    VVI_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(VVI_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(VVI_M, "stopping the model");
  runModel = 0;
}

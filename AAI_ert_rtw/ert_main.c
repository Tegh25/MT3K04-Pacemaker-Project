...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(AAI_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    AAI_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(AAI_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(AAI_M, "stopping the model");
  runModel = 0;
}
